...
// 💡 【安全新增】防護罩：只在 Electron 環境中載入 app 模組，避免普通 Node.js 測試時閃退
...
function initialize() {
  // 💡 【關鍵修改】智慧判斷環境：自動切換資料庫位置，徹底解決 Windows 打包後的唯讀權限問題
  let dbPath;
  if (electronApp && electronApp.isPackaged) {
    // 1️⃣ 正式打包環境：將資料庫檔案移往 Windows 擁有最高讀寫權限的 AppData 專屬資料夾
    // 實體路徑在：C:\Users\使用者名稱\AppData\Roaming\Hermes庫存管理系統\database.sqlite
    dbPath = path.join(electronApp.getPath('userData'), 'database.sqlite');
  } else {
    // 2️⃣ 平常本地開發測試區：退回上一層，維持你原本習慣的資料夾與檔名
